...
_backend: WebNavigatorBackend | None = None


def _get_backend() -> WebNavigatorBackend:
    global _backend
    if _backend is None:
        _backend = SeleniumChromeBackend()
    return _backend
...
def list_pages() -> list[dict]:
    ...
    return [p.__dict__ for p in _get_backend().list_pages()]
...
def new_page(url: str | None = None) -> dict:
    ...
    return _get_backend().new_page(url).__dict__


@mcp.tool()
def close_page(page_id: str) -> dict:
    """Close a tab by id."""
    _get_backend().close_page(page_id)
...
def select_page(page_id: str) -> dict:
    ...
    _get_backend().select_page(page_id)
...
def navigate(url: str) -> dict:
    ...
    url = validate_url(url)
    return _get_backend().navigate(url).__dict__
