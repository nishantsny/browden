...
- **Open tabs.** Every `navigate` / `new_page` leaves a tab open. An agent that
  reads a page and moves on would otherwise leak tabs for the life of the
  process.
- **Parsed-HTML caches.** Each DOM-query tool parses `driver.page_source` once
  ...
  the page changes, expire when it gets old, and not outlive the tab itself —
...
- **`PageRegistry`** (`web_navigator/registry.py`) — `page_id → last_access`,
  ...
  it. `tracked_ids()` lists what we currently track; `idle_pages(ttl)` returns
...
cleanup ultimately calls `backend.close_page(pid)` to actually close a tab.
...
| `new_page(url)`                   | invalidate the new tab's cache (no stale entry); `touch`  |
| `close_page(page_id)`             | `backend.close_page`; invalidate cache; `forget` registry |
| `select_page(page_id)`            | `touch` the registry                                      |
| `list_pages()`                    | `touch` every returned tab                                |
| `force_reload_page(page_id)`      | reload the tab in the browser; re-parse + store; `touch`  |
...
backend translates that to a domain `PageNotFoundError` with a one-line
message; `PageSession` catches it, drops the tab from the cache and the
registry, and the DOM tools return
`{"error": "page <id> is no longer open …", "page_id": <id>}` (mirroring the
invalid-CSS error shape). `close_page` on a gone tab is a no-op success;
`select_page` re-raises (you can't focus a tab that isn't there).
...
miss, TTL-expired stale-reload, or `force_reload_page`). That's the cache
...
afterwards — there's `PageSession.sweep_idle()`:

1. **Reconcile.** `live = backend.list_page_ids()` (a cheap call — just the
   handles, no per-tab focus changes; distinct from the heavy `list_pages()`).
...
2. **Reap the idle.** For each `pid` in `registry.idle_pages(IDLE_TTL_SECONDS)`
   (1 hour by default), call `backend.close_page(pid)` (swallow any error — the
...
a tool that touches it again (immediate, via `PageNotFoundError`), the next
...
- **(a) Reaper task vs an in-flight tool's driver op.** Selenium is synchronous
  ...
  once. So a reaper tick that fires *while* a tool's `close_page` / `navigate` /
  page-source fetch is in flight must not issue its own `close_page` /
  `list_page_ids` against that same driver.
...
dispatched via `PageSession._run_driver`:
...
When `sweep_idle` *does* proceed, its driver calls (`list_page_ids` for the
...
web_navigator/interface.py        WebNavigatorBackend, PageNotFoundError;
                                  list_page_ids() is the cheap "what tabs exist"
```
