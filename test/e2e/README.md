...
| `test_dom_cache_invalidation.py` | session-mgr + server | `invalidate_dom_cache`: a page-side DOM change is stale until invalidated; invalidate keeps live DOM state that force_reload discards; closed tab → tab-gone; the drop is scoped to the named tab; not read-gated, but the read that follows still is |
