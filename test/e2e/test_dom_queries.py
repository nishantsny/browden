...
These go through ``PageSession`` — the same path the MCP tools take — so they
...
def session(tmp_path):
    backend = SeleniumChromeBackend(profile_dir=str(tmp_path / "profile"), id_namespace="e2e")
...
async def test_get_element_by_id(session):
    page = await session.new_page(DATA_URL)

    res = await session.get_element_by_id("logo", page_id=page.id)
    ...
    missing = await session.get_element_by_id("nope", page_id=page.id)
...
async def test_get_elements_by_class_name(session):
    ...
    res = await session.get_elements_by_class_name("item js-item", page_id=page.id)
...
async def test_query_selector_single_match(session):
    ...
    res = await session.query_selector("p.note", page_id=page.id)
...
async def test_query_selector_all_paginates(session):
    ...
    first = await session.query_selector_all("li.item", page_id=page.id, limit=2, offset=0)
    assert first["total_count"] == 3
    assert first["returned"] == 2
    assert first["next_offset"] == 2

    rest = await session.query_selector_all("li.item", page_id=page.id, limit=2, offset=2)
...
async def test_query_selector_invalid_css_is_structured_error(session):
    ...
    res = await session.query_selector("div::::bad", page_id=page.id)
...
async def test_force_reload_page(session):
    ...
    await session.get_element_by_id("logo", page_id=page.id)

    res = await session.force_reload_page(page_id=page.id)
    ...
    again = await session.get_element_by_id("logo", page_id=page.id)
