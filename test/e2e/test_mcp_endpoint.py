...
async def test_mcp_endpoint(mcp_server, mcp_client_session):
    async with mcp_client_session(mcp_server) as mcp_client:
        ...
        expected_tools = {
            ...
            "click", "get_element_by_id", "get_elements_by_class_name",
        ...
        assert any(p.get("id") == tab_id for p in list_pages_items)
    
        # query_selector("html") on the new page -> found (about:blank still has a root element).
        query_res = await mcp_client.call_tool("query_selector", {"css_selector": "html", "id": tab_id})
        ...
        # close_tab works
        close_res = await mcp_client.call_tool("close_tab", {"id": tab_id})
