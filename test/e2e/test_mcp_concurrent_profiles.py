...
async def test_concurrent_profiles_are_isolated(mcp_server, mcp_client_session, tmp_path):
    async with mcp_client_session(mcp_server) as session1, \
        ...
        pages = _pages_from(await session1.call_tool("list_tabs", arguments={}))
        ...
        # 3. Concurrent DOM reads, one per profile, routed by id alone.
        dom_results = await asyncio.gather(
            session1.call_tool("query_selector", arguments={"css_selector": "body", "id": id1}),
            session2.call_tool("query_selector", arguments={"css_selector": "body", "id": id2})
        )
        ...
        # 4. Garbage id returns the error envelope, not a crash.
        garbage = await session1.call_tool(
            "query_selector", arguments={"css_selector": "body", "id": "deadbeef-123"})
        err = json.loads(garbage.content[0].text)
        assert "error" in err
        assert err["id"] == "deadbeef-123"
